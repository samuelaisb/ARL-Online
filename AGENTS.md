...
\*Both `SUPABASE_URL` and anon key are required for header auth to appear; omit either to hide auth UI. \*`SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` are required for the Express server (inventory). \*Member approval/refusal emails require `RESEND_API_KEY` at send time.
...
| 2026-06-16 | Removed admin Resend notification on new pending reservations (Slack webhook only); removed `EMAIL_TO` / `RESERVE_INVENTORY_EMAIL_TO` env vars. Member approve/refuse emails still go to reservation `userEmail` from JWT. |
| 2026-06-16 | Google Analytics tag (`G-5VERECD6ZJ`) in `index.html` (all routes via SPA shell); CSP updated for gtag; SPA route-change pageviews in `App.svelte`. |
